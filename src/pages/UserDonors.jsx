...
import BloodBankTable from "../components/DonorsTable";
const BloodBank = () => {
  ...
  useEffect(() => {
    const getUserData = async () => {
      const res = await axios.get(
        `http://localhost:8080/get_remaining_donors?my_id=${
          JSON.parse(Cookies.get("user_data")).id
        }`
      );
      ...
      <div>
        <div className="flex flex-row ">
          ...
          <div className="w-full ">
            <DashBoardHeader />
            <DashboardTitleHeader module_name={"Blood Banks"} />
            <div className="m-5 bg-gray-200 p-3 rounded-md shadow-lg shadow-gray-700 ">
              <BloodBankTable hospitalData={hospitalData} />
            </div>
          </div>
        </div>
      </div>
